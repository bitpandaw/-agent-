...
1. **planner和executor耦合重**:planner目前并没有实现多步规划，实际上并不是planner，executor和planner通过conversation通讯，设计不够好
2. **RAG 分块过于粗糙**:rag的文件检索质量严重受到文档的约束，并没有考虑到真实工业场景下的数据
3. **无评估体系**:没有建立正确的评估体系
4. **数据来源不是真实的工业数据**:真实的工业数据需要用到数据清洗技术，目前并没有找到真实的数据来源

### 技术选型理由

- DeepSeek：我没有visa卡，购买不了openai，且我花了300元/月购买Claude code，以及代理服务器，没有钱了
